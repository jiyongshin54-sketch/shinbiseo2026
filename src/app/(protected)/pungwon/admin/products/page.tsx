...
import { PRICE_GRADES, SELLER_COMPANIES } from '@/lib/constants'
...
// 품명 기반 카테고리 자동 배정 (원본 Pungwon.aspx.cs 로직)
function getCategoryId(attribute01: string): string {
  if (attribute01 === '손잡이') return '00002'
  if (attribute01 === '칼라') return '00003'
  if (attribute01 === '이중') return '00004'
  return '00001'
}

export default function PungwonAdminProductsPage() {
  ...
  const isPungwonAdmin = isSeller && user?.companyId === SELLER_COMPANIES.PUNGWON && (isOwner || isAdmin)

  const fetchProducts = useCallback(async () => {
    if (!user) return
    setLoading(true)
    try {
      const res = await fetch(`/api/products?seller_id=${SELLER_COMPANIES.PUNGWON}`)
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const categoryId = getCategoryId(form.attribute01)
      ...
      let res: Response
      if (editingProductId) {
        res = await fetch(`/api/products/${SELLER_COMPANIES.PUNGWON}/${editingProductId}`, {
          method: 'PATCH',
        ...
        res = await fetch('/api/products', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ seller_id: SELLER_COMPANIES.PUNGWON, ...payload }),
        })
      }
  ...
  const handleDelete = async (p: ProductRow) => {
    ...
    try {
      const res = await fetch(`/api/products/${SELLER_COMPANIES.PUNGWON}/${p.product_id}`, { method: 'DELETE' })
      const result = await res.json()
      if (result.error) throw new Error(result.error)
      toast.success('삭제 완료')
      fetchProducts()
  ...
  if (!user) return null
  if (!isPungwonAdmin) {
    return <div style={{ padding: '40px', textAlign: 'center', color: '#999' }}>풍원 기성 상품 관리는 풍원 대표/관리자만 사용 가능합니다.</div>
  }
  ...
  return (
    <div>
      ...
      <div style={{ backgroundColor: 'cadetblue', padding: '8px 15px', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
        <span style={{ fontSize: '18px', fontWeight: 'bold' }}>풍원 기성 상품 관리</span>
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={() => router.push('/pungwon')} style={{ padding: '4px 16px', cursor: 'pointer' }}>
            풍원으로 돌아가기
          </button>
        </div>
      </div>
      ...
      {/* 상품 목록 */}
      <div style={{ maxHeight: '1200px', overflowY: 'auto', overflowX: 'auto', border: '1px solid silver', marginBottom: '8px' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={p.product_id} style={{ borderBottom: '1px solid #ddd' }} className="hover:bg-gray-50">
                ...
                <td style={{ ...tdStyle, textAlign: 'center' }}>{p.category_id}</td>
      ...
      {formOpen && (
        <div style={{ backgroundColor: 'palegoldenrod', padding: '10px', border: '2px solid #999' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '6px' }}>
            {editingProductId && (
              <div>
                <div style={labelStyle}>상품ID</div>
                <input value={editingProductId} readOnly style={{ width: '60px', ...inputStyle, backgroundColor: '#eee' }} />
              </div>
            )}
            <div>
